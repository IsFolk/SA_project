...
public class TestHelper {
    ...
    public static TestHelper getHelper() {
        if(th == null) th = new TestHelper();
        
        return th;
    }
    
    public JSONObject create(Test test) {
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        int id = -1;
        ...
        response.put("test_id", id);
        response.put("question_id", qa);
    ...
    public JSONObject getById(String test_id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa_project`.`test` WHERE `test`.`TestId` = ?";
            
            /** 將參數回填至SQL指令當中，若無則不用只需要執行 prepareStatement */
            pres = conn.prepareStatement(sql);
            pres.setString(1, test_id);
            /** 執行查詢之SQL指令並記錄其回傳之資料 */
            rs = pres.executeQuery();
    ...
    public JSONObject deleteByID(int id) {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "DELETE FROM `sa_project`.`test` WHERE `TestId` = ? LIMIT 1";
     ...
     */
    public JSONObject update(Test t) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
